...
class RRTConnect:
    ...
    def plan(self, obstacle_points, robot_radius,
             max_iter=RRT_MAX_ITER, time_budget_ms=None):
        """
        Trả về (success, path, tree_a, tree_b, iters_used).
        Path theo thứ tự start -> goal.

        time_budget_ms: nếu set, dừng khi vượt budget (cho MPC real-time).
                       Check mỗi 10 iter để giảm overhead.
        """
        ...
        for it in range(max_iter):
            # Time budget check (mỗi 10 iter để giảm overhead time.time())
            if time_budget_ms is not None and (it % 10 == 0):
                if (_time.time() - t_start) * 1000 > time_budget_ms:
                    break
...
def _path_to_arclength(path_arr):
    """Tính arc-length cumulative cho 1 path numpy (N, 2)."""
...
def _check_spline_collision(points, obstacle_points, robot_radius):
    """Check collision dọc theo các segment của spline."""
    for i in range(len(points) - 1):
        if RRTConnect._collides(Node(tuple(points[i])),
                                Node(tuple(points[i + 1])),
                                obstacle_points, robot_radius,
                                ignore_start=(i == 0)):
            return True
    return False
...
def smooth_path_spline(path, obstacle_points, robot_radius,
                       num_points=None, density=1.0,
                       check_collision=True):
    """
    Mượt hóa path bằng cubic spline, parameterize theo arc-length.

    Args:
        path: list các [x, y] (start -> goal)
        obstacle_points: lidar hits (để check collision sau khi mượt)
        robot_radius: bán kính robot
        num_points: số điểm output. Nếu None -> auto theo density.
        density: số điểm trên 1 đơn vị arc-length (chỉ dùng khi num_points=None)
        check_collision: nếu True và spline đụng obstacle -> trả về path gốc

    Returns:
        smoothed_path: numpy array (M, 2) — path đã mượt
                       hoặc path gốc (dạng list) nếu spline fail/collision
    """
    if path is None or len(path) < 2:
        return path
    ...
    # Khử các điểm trùng nhau (spline sẽ crash nếu có)
    keep = [0]
    for i in range(1, len(arr)):
        if np.hypot(arr[i, 0] - arr[keep[-1], 0],
                    arr[i, 1] - arr[keep[-1], 1]) > 1e-6:
            keep.append(i)
    arr = arr[keep]
    ...
    # Tính arc-length parameter
    s = _path_to_arclength(arr)
    total_len = s[-1]
    if total_len < 1e-6:
        return path
    ...
    if num_points is None:
        num_points = max(int(total_len * density), n * 2)
    num_points = max(num_points, 2)
    s_fine = np.linspace(0, total_len, num_points)

    # n == 2: chỉ có thể linear interpolation
    if n == 2:
        t = s_fine / total_len
        smoothed = np.column_stack([
            arr[0, 0] + t * (arr[1, 0] - arr[0, 0]),
            arr[0, 1] + t * (arr[1, 1] - arr[0, 1])])
        return smoothed

    # n >= 3: cubic spline. Dùng natural BC (curvature = 0 ở 2 đầu)
    try:
        from scipy.interpolate import CubicSpline
        cs_x = CubicSpline(s, arr[:, 0], bc_type='natural')
        cs_y = CubicSpline(s, arr[:, 1], bc_type='natural')
        smoothed = np.column_stack([cs_x(s_fine), cs_y(s_fine)])
    except ImportError:
        # Fallback: numpy interpolation tuyến tính nếu không có scipy
        smoothed = np.column_stack([
            np.interp(s_fine, s, arr[:, 0]),
            np.interp(s_fine, s, arr[:, 1])])

    # Collision check
    if check_collision and obstacle_points is not None \
            and len(obstacle_points) > 0:
        if _check_spline_collision(smoothed, obstacle_points, robot_radius):
            # Spline lượn vào obstacle -> trả về path gốc
            return path

    return smoothed


def path_for_mpc(path, obstacle_points, robot_radius, horizon_length):
    """
    Convenience function: resample path đúng số điểm = horizon_length
    để feed thẳng vào MPC reference.
    """
    smoothed = smooth_path_spline(path, obstacle_points, robot_radius,
                                  num_points=horizon_length,
                                  check_collision=True)
    # Nếu spline bị reject (trả về list path gốc), interpolate tuyến tính
    if isinstance(smoothed, list):
        arr = np.array(smoothed, dtype=float)
        s = _path_to_arclength(arr)
        s_fine = np.linspace(0, s[-1], horizon_length) if s[-1] > 1e-6 \
            else np.zeros(horizon_length)
        smoothed = np.column_stack([
            np.interp(s_fine, s, arr[:, 0]),
            np.interp(s_fine, s, arr[:, 1])])
    return smoothed
