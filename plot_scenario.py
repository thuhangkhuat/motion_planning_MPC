...
def parse_args():
    ...
    for a in sys.argv[1:]:
        if a == '--traj':
            ...
            scenario = a      # number (scen<N>.yaml) or scenario name
...
def main():
    ...
    from config import (STARTS, XLIM, YLIM, SCENARIO_NAME, SCENARIO_DEF, GENERATION_STATS)
    ...
    starts = STARTS
    rects = SCENARIO_DEF['rects']
    circles = SCENARIO_DEF['circles']
    xlim, ylim = XLIM, YLIM
    scen_no = SCENARIO_NAME
    if GENERATION_STATS is not None:
        print(f"Generated map: {GENERATION_STATS['n_rects']} rects + "
              f"{GENERATION_STATS['n_circles']} circles")
    ...
    ax.set_title(f"{scen_no} — {st.shape[0]} UAV, "
                 f"{n_obs} obstacles, density {density:.1f}%")
    ax.legend(loc='best', fontsize=9)
    ax.grid(alpha=0.3)
    plt.tight_layout()

    out_png = f"{scen_no}_preview.png"
