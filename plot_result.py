...
# TÙY CHỈNH HIỂN THỊ
# ============================================================
DRONE_COLOR = "#1f77b4"     # MỌI drone cùng màu
# DRONE_COLOR = ['#1f77b4', '#ff7f0e', '#d62728', '#9467bd',
#           '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']
FOV_COLOR   = "#2ca02c"     # màu FOV (dùng chung)
LEADER_COLOR = "#f1c40f"

FOV_SPACING = 0.75          # m — rải 1 ô FOV mỗi 0.75m đường đi (nhỏ = dày hơn)
...
def draw_drone(ax, x, y, yaw=0.0, scale=1.0, color=DRONE_COLOR,
...
# ============================================================
if SHOW_2D:
    size = (12, 7.5) if SCENARIO in (7, 8) else (15, 7.5)
    ...
    for j in range(OBSTACLES.shape[0]):
        x, y, r = OBSTACLES[j, :]
        a, b = getCircle(x, y, r)
        ax.fill(a, b, color="black", label="Obstacle" if j == 0 else "")
    ...
    for i in range(NUM_ROBOT):
        ...
        ax.plot(path[:, 1], path[:, 2], color=DRONE_COLOR, lw=1.3,
                label="UAV path" if i == 0 else "")
        ...
        draw_drone(ax, pos[0], pos[1], yaw, scale=1.0, color=DRONE_COLOR,
    ...
    ax.plot([], [], color=FOV_COLOR, lw=6, alpha=0.35, label="FOV")
...
if SHOW_3D:
    ...
    for i in range(NUM_ROBOT):
        ...
        zcol = np.where(np.abs(path[:length, 3]) > 1e-6, path[:length, 3],
                        Z_FALLBACK)
        ax3.plot(path[:length, 1], path[:length, 2], zcol, color=DRONE_COLOR,
                 lw=1.4, label="UAV path" if i == 0 else None)
        ...
        for cx, cy in [(x - L, y - L), (x + L, y - L),
            ...
            ax3.plot([x, cx], [y, cy], [z, 0], color=FOV_COLOR, lw=0.6,
                     alpha=0.5)
        ax3.scatter(x, y, z, marker="X", s=70, c=DRONE_COLOR,
    ...
    ax3.plot([], [], color=FOV_COLOR, label="FOV footprint")
    ax3.scatter([], [], marker="X", c=DRONE_COLOR, label="UAV")
    ax3.set_xlim(XLIM)
    ax3.set_ylim(YLIM)
    ...
    ax3.set_ylabel("y [m]")
    ax3.set_zlabel("z [m]")
    ax3.set_box_aspect((XLIM[1] - XLIM[0], YLIM[1] - YLIM[0], OBS_HEIGHT + 1))
