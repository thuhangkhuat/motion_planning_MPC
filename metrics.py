...
def fov_coverage(centers, side, target=None,
                 connect_slack=None, mode="drop_isolated"):
    """
    ...
    connect_slack: khe hở tối đa để 2 FOV vẫn coi là liên thông
                   (mặc định 0.5 * side/2 = 0.25*side, tránh brittle khi
                    UAV lệch nhẹ khỏi thế tile khít).
    ...
    if mode == "drop_isolated":
        ...
        h = side / 2.0
        # component nào chứa target (target nằm trong 1 FOV của nó)
        kept = []
        for comp in comps:
            for i in comp:
...
def run_metrics(data, fov_side, robot_radius, timestep,
                cov_mode="drop_isolated", connect_slack=None,
                warmup_steps=0):
    """
    ...
    for t in range(T):
        area, _ = fov_coverage(pos[t], fov_side, target=tar[t],
